...
Основной исходный код собран в поддиректории `src/graphcoloring`.
В директории `tests` лежат тесты.

В директории `src/graphcoloring` лежат четыре файла:
1. `Mesh3D.py` - содержит класс, реализующий 3D меш. Нужна для хранения разбиения Вороного.
2. `SphericalNetwork.py` - содержит класс, реализующий всю сеть целиком. Класс хранит разбиение Вороного в `Mesh3D` и граф связей в `networkx.Graph`. Также, в этом классе реализованы методы построения изначальных графа и меша.
3. `coloring.py` - содержит все необходимые методы для раскрасок и рассчётов, связаных с раскрасками графов. Основные перечислены ниже:
    1. `graphDistanceColoring` - жадный алгоритм раскрашивания
    2. `graphDistanceColoringDsatur` - алгоритм ракрашивания *DSATUR*. Раскрашивает точнее, но требует больше времени, особенно на больших данных
    3. `calcMaxColoringDistance` - алгоритм максимизации минимального расстояния раскраски
    4. `calcMinimumColors` - минимизация количества цветов для раскраски графа. Дистанция передаётся как параметр
4. `draw.py` - содержит методы отрисовки графов, 3D меша и класса `SphericalNetwork`.

---

### Картинки

**Точки и их граф связей**

Картинки приведены для $100$ точек и $16$ цветов


![Точки и их граф связей](images/networkGraph.png)

**Разбиение сферы на области Вороного**
...
Эксперимент проводился для жадной стратегии раскрашивания.

| Вершины\цвета | 20           | 100          | 500          | 1008          |
| ------------- | ------------ | ------------ | ------------ | ------------- |
| 100           | >1 сек       | >1 сек       | >1 сек       | >1 сек        |
| 1000          | 2 сек        | 3 сек        | 20 сек       | 22 сек        |
| 10000         | 8 сек        | 25 сек       | 6 мин 11 сек | 12 мин 48 сек |
| 64000         | 1 мин 16 сек | 3 мин 15 сек | 1 час 3 мин  | 1 час 20 мин  |
